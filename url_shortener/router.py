...
@url_shortener.get("/list", response_model=list)
async def list():
...
# Redirect the user to the original url
@url_shortener.get("/go/{short_url}")
async def go(short_url : str):
    # Get the url from the database
    url = await mock_db_operations.fetch_data(short_url) 
    # Check if the url exists in the database
    if bool(url) & bool(url!=''):
        # return the redirect response
        response = RedirectResponse(url=url.decode('utf-8'))
        return response
    else:
        # return the error message
        return {"message": "Failed to fetch"}
